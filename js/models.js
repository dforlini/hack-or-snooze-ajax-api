...
class Story {
  ...
  getHostName() {
   try {
    ...
    console.error('Failed to parse hostname', error);
    return 'unknown';
   }
  }
}
...
class StoryList {
  ...
  async addStory( user, newStoryData ) {
    ...
    console.error('error in adding story', error);
    throw error;
  }
}
...
isFavorite(storyId){
 ...
 try{  await axios.post(`${BASE_URL}/users/${this.username}/favorites/${storyId}`,{
    ...
    console.error('story not found in global list');
  }
}catch (error){
